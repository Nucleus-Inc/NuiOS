...
public extension UIView{
    ...
    @discardableResult
    class public func showAlert(OnVC viewController:UIViewController!,WithTitle title:String?,Message message:String?,AndActions actions:[UIAlertAction])->UIAlertController{
    ...
    class public func showAlertWithTF(onVC vc:UIViewController!,WithTitle title:String?,Message message:String?,Actions actions:[UIAlertAction],AndTextFieldBlock block:@escaping (_ textField:UITextField)->Void){
        let alert = UIAlertController(title: title, message: message, preferredStyle: UIAlertControllerStyle.alert)
        
        alert.addTextField(configurationHandler: block)
    ...
    /*
    class public func showLoadingAlert(OnVC viewController:UIViewController!,WithTitle title:String)->UIAlertController{
        let pending = UIAlertController(title: title, message: nil, preferredStyle: .alert)
        
        let indicator = UIActivityIndicatorView()
        indicator.color = UIColor.darkGray
        indicator.translatesAutoresizingMaskIntoConstraints = false
        pending.view.subviews[0].addSubview(indicator)
        pending.view.subviews[0].sizeToFit()
        let width = NSLayoutConstraint(item: indicator, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 50)
        indicator.addConstraint(width)
        let height = NSLayoutConstraint(item: indicator, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 50)
        indicator.addConstraint(height)
        
        
        let centerX = NSLayoutConstraint(item: indicator, attribute: .centerX, relatedBy: .equal, toItem: pending.view, attribute: .centerX, multiplier: 1, constant:-50)
        pending.view.addConstraint(centerX)
        let centerY = NSLayoutConstraint(item: indicator, attribute: .centerY, relatedBy: .equal, toItem: pending.view, attribute: .centerY, multiplier: 1, constant: 0)
        pending.view.addConstraint(centerY)
        ...
        /*let pending = UIAlertController(title: nil, message: title, preferredStyle: .alert)
        //create an activity indicator
        let indicator = UIActivityIndicatorView(frame: CGRect(x: 30, y: 30, width: 50, height: 50))
        indicator.color = UIColor.darkGray
        indicator.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        
        //add the activity indicator as a subview of the alert controller's view
        pending.view.addSubview(indicator)
        //pending.view.frame.size = CGSize(width: 10, height: pending.view.frame.height)
